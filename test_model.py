...
def retrieve_images(img, embed_100, embed_4096, hash, label, topk):
    ...
    with torch.no_grad():
        ...
        for i in range(topk):
            print(
                f"Similarity:{sim_100_d[i][0]} Label:{ds_label['fine_label_names'][trained_img_labels[sim_100_d[i][1]]]}")
            res = (ds_label['fine_label_names'][trained_img_labels[sim_100_d[i][1]]]
                   == ds_label['fine_label_names'][label])
            if i < 1:
                p1_1 += res
            elif i < 5:
                p1_5 += res
            ...
            x = class_embeds['embedding'][trained_img_labels[sim_100_d[i][1]]]
        ...
        for i in range(topk):
            print(
                f"Similarity:{sim_hash[i][0]} Label:{ds_label['fine_label_names'][trained_img_labels[sim_hash[i][1]]]}")
        ...
        for i in range(2*topk):
            sim_100_d_hash.append(((F.cosine_similarity(
                torch.from_numpy(trained_img_embed_100[sim_hash[i][1]]), embed_100)).numpy(), sim_hash[i][1]))
        ...
        for i in range(topk):
            print(
                f"Similarity:{sim_100_d_hash[i][0]} Label:{ds_label['fine_label_names'][trained_img_labels[sim_100_d_hash[i][1]]]}")
            res = (ds_label['fine_label_names']
                   [trained_img_labels[sim_100_d_hash[i][1]]] == ds_label['fine_label_names'][label])
            if i < 1:
                p2_1 += res
            elif i < 5:
                p2_5 += res
                ...
                p2_20 += res
            x = class_embeds['embedding'][trained_img_labels[sim_100_d_hash[i][1]]]
            y = class_embeds['embedding'][label]
            class_similar.append(cosine_similarity(
                x.reshape(1, -1), y.reshape(1, -1)))
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        for img, label in image_datasets["test"]:
            ...
            hash = torch.unsqueeze(torch.from_numpy(hash),0)
